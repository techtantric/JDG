...
//      			.host(host).port(hotrodPort)
   ...
   public String mainMethod() throws Exception {
      forecast = new SnowForecast();
      return "";
   }
   
   void actONdata(String action) {
   ...
   public void addForecastNew(Forecast ft) {
	      if (remoteCache.containsKey(ft.getId())) {
	         System.out.println("Updating forecast with id " + ft.getId());
	      }

	      // put the Forecast in cache
	      remoteCache.put(ft.getId(), ft);
	   }
   
   public void removeForecastNew(Integer id) {
//	      int id = Integer.parseInt(readConsole("Enter forecast id to remove (int): "));
   ...
   public String printSnowReportNew() {
	      QueryFactory qf = Search.getQueryFactory(remoteCache);
	      Query q = qf.from(Forecast.class)
	            .select(Expression.property("location"), Expression.property("year"), Expression.property("month"),
	                  Expression.property("day"), Expression.sum("rain"), Expression.sum("snowfall"),
	   ...
	   public String printAllEntriesNew() {
	      for (Object key : remoteCache.keySet()) {
	         System.out.println("key=" + key + " value=" + remoteCache.get(key));
	      }
	      
	      return remoteCache.keySet().toString();
	   }
